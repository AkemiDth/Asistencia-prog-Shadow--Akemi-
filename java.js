...
function gettask() {
 ...
 for (let i = 0; i < tasks.length; i++) {
  ...
  viewtask.innerHTML += `
     <div class = "card mb-4"> 
     <div class= "card-body">
     <p> ${namebud}: Asistencia:${monuse}- Nota = ${initbud}  - Dia de clase:${date}<p><hr>
     <p>Apreciativa del estudiante: ${desc}<p>
     <div id="changes">
        <div id="edit">
        </div>
     </div>
     ...
     <hr>
     <a class="btn btn-danger" onclick= "deltask('${namebud}')">Quitar</a>
     </div>`
 }
 gettask()
}
